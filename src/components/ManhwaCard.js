      ...
      <Link href={`/series/${series.slug || series.id}`} className="relative block aspect-[3/4] w-full overflow-hidden bg-[var(--bg-surface)]">
        ...
        <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between z-10">
          {rank ? (
            ...
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase bg-black/60 backdrop-blur-md text-white border border-white/10">
              {series.status || 'Ongoing'}
            </span>
          )}

          {/* Bookmark Button with tactile feedback */}
          <button
        ...
        <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center justify-between text-white text-[11px] font-medium z-10">
          {series.rating && (
            <span className="flex items-center gap-1 bg-black/65 backdrop-blur-md px-2 py-0.5 rounded-lg border border-white/10">
